...
키움증권, LS증권, DB증권, 한국투자증권의 공식 Open API 문서를 AI 코딩 참고용 Markdown 레퍼런스와 공통 SDK metadata로 생성하는 저장소입니다.
...
core 구현 이후 외부 서버/앱 연동을 위한 안정화 설계는 [docs/sdk-stabilization-goal-plan.md](docs/sdk-stabilization-goal-plan.md), [docs/live-integration-readiness.md](docs/live-integration-readiness.md), [docs/public-sdk-contract.md](docs/public-sdk-contract.md), [docs/production-readiness-checklist.md](docs/production-readiness-checklist.md)에 정리되어 있습니다.

SDK 구현은 `data/generated/kiwoom-manifest.json`, `data/generated/ls-manifest.json`, `data/generated/db-manifest.json`, `data/generated/kis-manifest.json`을 기준으로 API ID/TR 코드, 엔드포인트, 헤더, 요청/응답 필드, 인증 필요 여부를 조회하도록 설계합니다.
...
const kisPrice = registry.getEndpoint("kis", "/uapi/domestic-stock/v1/quotations/inquire-price");
```
...
DB증권 OPEN API는 LS형 TR 코드 기반 구조를 별도 client로 호출합니다.

```js
import { DbClient } from "security-api-reference";
...
const result = await db.request("PRICE", {
  In: {
...
한국투자증권 Open API는 REST path 기반으로 호출합니다. 실시간 WebSocket 접속키는 REST access token과 별도 approval key 흐름을 사용합니다.

```js
import { KisClient } from "security-api-reference";
...
const result = await kis.request("/uapi/domestic-stock/v1/quotations/inquire-price", {
...
국내주식 시세 조회 도메인 서비스는 4개 증권사의 현재가, 호가, 복수 현재가 조회를 공통 형태로 제공합니다.
시장 데이터 서비스는 4개 증권사의 종목 기본정보, 일봉/분봉 OHLCV 조회를 공통 형태로 제공합니다.
