---
broker: "LS증권"
source_url: "https://openapi.ls-sec.co.kr/apiservice?group_id=2f1eea77-5606-4512-93c6-31b21d2ece90&api_id=57936c91-b49d-4702-b7f6-3935c6859462"
scraped_at: "2026-05-22T15:19:33.392Z"
category: "선물/옵션"
api_id: "57936c91-b49d-4702-b7f6-3935c6859462"
api_name: "[선물/옵션] 실시간 시세"
tr_id: "futroptn-0000-0000-0000-000000000C02"
tr_code: "C02"
method: "POST"
domain: "wss://openapi.ls-sec.co.kr:9443"
path: "/websocket/futureoption"
content_type: "application/json; charset=UTF-8"
rate_limit: "-"
auth_required: true
---

# KRX야간파생 선물체결 (C02)

<!-- request_field_count: 4 -->
<!-- response_field_count: 29 -->

## 요약

| 항목 | 값 |
| --- | --- |
| 그룹 | 선물/옵션 |
| API 페이지 | [선물/옵션] 실시간 시세 |
| TR명 | KRX야간파생 선물체결 |
| TR코드 | `C02` |
| 초당 전송 건수 | - |
| 설명 | 선물옵션 주문현황 및 시세, 투자정보를 실시간으로 확인할 수 있습니다. |

## 엔드포인트

| 항목 | 값 |
| --- | --- |
| Protocol | WEBSOCKET |
| Method | POST |
| 운영 도메인 | `wss://openapi.ls-sec.co.kr:9443` |
| 모의투자 도메인 | `wss://openapi.ls-sec.co.kr:29443` |
| URL | `/websocket/futureoption` |
| Request Format | JSON |
| Content-Type | application/json; charset=UTF-8 |

## 인증/헤더

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 설명 |
| --- | --- | --- | --- | --- | --- | --- |
| Request Header | `token` | 접근토큰 | String | Y | 1000 | - |
| Request Header | `tr_type` | 거래 Type | String | Y | 1 | - |

## 요청

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 설명 |
| --- | --- | --- | --- | --- | --- | --- |
| Request Body | `tr_cd` | 거래 CD | String | Y | 3 | - |
| Request Body | `tr_key` | 단축코드 | String | N | 8 | - |

## 응답

### 헤더

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 설명 |
| --- | --- | --- | --- | --- | --- | --- |
| Response Header | `tr_cd` | 거래 CD | String | Y | 3 | - |

### Body

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 설명 |
| --- | --- | --- | --- | --- | --- | --- |
| Response Body | `lineseq` | 라인일련번호 | String | Y | 10 | - |
| Response Body | `accno` | 계좌번호 | String | Y | 11 | - |
| Response Body | `user` | 조작자ID | String | Y | 8 | - |
| Response Body | `seq` | 일련번호 | String | Y | 11 | - |
| Response Body | `trcode` | trcode | String | Y | 11 | - |
| Response Body | `megrpno` | 매칭그룹번호 | String | Y | 2 | - |
| Response Body | `boardid` | 보드ID | String | Y | 2 | - |
| Response Body | `memberno` | 회원번호 | String | Y | 5 | - |
| Response Body | `bpno` | 지점번호 | String | Y | 5 | - |
| Response Body | `ordno` | 주문번호 | String | Y | 10 | - |
| Response Body | `ordordno` | 원주문번호 | String | Y | 10 | - |
| Response Body | `expcode` | 종목코드 | String | Y | 12 | - |
| Response Body | `yakseq` | 약정번호 | String | Y | 11 | - |
| Response Body | `cheprice` | 체결가격 | String | Y | 11.2 | - |
| Response Body | `chevol` | 체결수량 | String | Y | 10 | - |
| Response Body | `sessionid` | 세션ID | String | Y | 2 | - |
| Response Body | `chedate` | 체결일자 | String | Y | 8 | - |
| Response Body | `chetime` | 체결시각 | String | Y | 9 | - |
| Response Body | `spdprc1` | 최근월체결가격 | String | Y | 11.2 | - |
| Response Body | `spdprc2` | 차근월체결가격 | String | Y | 11.2 | - |
| Response Body | `dosugb` | 매도수구분 | String | Y | 1 | - |
| Response Body | `accno1` | 계좌번호1 | String | Y | 12 | - |
| Response Body | `sihogagb` | 시장조성호가구분 | String | Y | 1 | - |
| Response Body | `jakino` | 위탁사번호 | String | Y | 5 | - |
| Response Body | `daeyong` | 대용주권계좌번호 | String | Y | 12 | - |
| Response Body | `mem_filler` | mem_filler | String | Y | 7 | - |
| Response Body | `mem_accno` | mem_accno | String | Y | 11 | - |
| Response Body | `mem_filler` | mem_filler1 | String | Y | 42 | - |

## 예제

### Request

```json
{
  "header": {
    "token": "토큰",
    "tr_type": "1"
  },
  "body": {
    "tr_cd": "C02",
    "tr_key": "101W6000"
  }
}
```

### Response

```json
{
  "header": {
    "tr_cd": "C02"
  },
  "body": {
    "accno": "***********",
    "mem_filler": "*****",
    "accno1": "0***********",
    "sessionid": "40",
    "sihogagb": "0",
    "trcode": "TTRTDP21301",
    "megrpno": "01",
    "memberno": "00063",
    "spdprc1": "0.00",
    "boardid": "G1",
    "spdprc2": "0.00",
    "seq": "69",
    "yakseq": "00000000613",
    "lineseq": "900000129",
    "bpno": "00201",
    "chevol": "1",
    "daeyong": "",
    "chetime": "161201555",
    "chedate": "20250610",
    "ordno": "0000000013",
    "expcode": "KR4101W60000",
    "mem_accno": "***********",
    "cheprice": "415.90",
    "jakino": "",
    "user": "",
    "dosugb": "1",
    "ordordno": ""
  }
}
```

## 연속조회/실시간/주의사항

- `transactionPerSec` 값은 공식 문서의 초당 전송 건수 원문입니다.
- 실시간 시세 TR은 그룹/문서명과 필드 구조를 기준으로 구분합니다.
- 빈 값은 공식 문서에 값이 없음을 의미합니다.

## 관련 오류

- LS증권 오류 정책은 API 페이지의 설명과 응답 예제를 우선합니다.
