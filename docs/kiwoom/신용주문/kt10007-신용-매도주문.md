...
| Body | `crd_deal_tp` | 신용거래구분 | String | Y | 2 | 99 | 33:융자 , 99:융자합 |
| Body | `crd_loan_dt` | 대출일 | String | N | 8 | - | YYYYMMDD(융자일경우필수) |
| Body | `dmst_stex_tp` | 국내거래소구분 | String | Y | 3 | KRX | KRX,NXT,SOR |
| Body | `ord_qty` | 주문수량 | String | Y | 12 | 3 | - |
| Body | `ord_uv` | 주문단가 | String | N | 12 | 6450 | - |
| Body | `stk_cd` | 종목코드 | String | Y | 12 | 005930 | - |
| Body | `trde_tp` | 매매구분 | String | Y | 2 | 0 | 0:보통 , 3:시장가 , 5:조건부지정가 , 81:장마감후시간외 , 61:장시작전시간외, 62:시간외단일가 , 6:최유리지정가 , 7:최우선지정가 , 10:보통(IOC) , 13:시장가(IOC) , 16:최유리(IOC) , 20:보통(FOK) , 23:시장가(FOK) , 26:최유리(FOK) , 28:스톱지정가,29:중간가,30:중간가(IOC),31:중간가(FOK) |
