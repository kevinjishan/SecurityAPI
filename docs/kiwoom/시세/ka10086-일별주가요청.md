...
| 항목 | 값 |
| --- | --- |
| 메뉴 위치 | 국내주식 > 시세 > 일별주가요청 |
| API ID | `ka10086` |
| 전송 방식 | REST |
| 설명 | [ 주의 ]  외국인순매수 데이터는 거래소로부터 금액데이터가 제공되지 않고 수량으로만 조회됩니다. |
...
| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 샘플 | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Body | `indc_tp` | 표시구분 | String | Y | 1 | 0 | 0:수량, 1:금액(백만원) |
| Body | `qry_dt` | 조회일자 | String | Y | 8 | 20241125 | YYYYMMDD |
