---
broker: "키움증권"
source_url: "https://openapi.kiwoom.com/guide/apiguide?dummyVal=0"
scraped_at: "2026-05-22T15:19:33.392Z"
category: "종목정보"
api_id: "ka10002"
api_name: "주식거래원요청"
method: "POST"
domain: "https://api.kiwoom.com"
path: "/api/dostk/stkinfo"
content_type: "application/json;charset=UTF-8"
rate_limit: "-"
auth_required: true
---

# 주식거래원요청 (ka10002)

<!-- request_field_count: 5 -->
<!-- response_field_count: 40 -->

## 요약

| 항목 | 값 |
| --- | --- |
| 메뉴 위치 | 국내주식 > 종목정보 > 주식거래원요청 |
| API ID | `ka10002` |
| 전송 방식 | REST |
| 설명 | 종목 코드로 거래원 정보를 조회합니다. |

## 엔드포인트

| 항목 | 값 |
| --- | --- |
| Method | POST |
| 운영 도메인 | `https://api.kiwoom.com` |
| 개발 도메인 | `https://apidev.kiwoom.com` |
| 모의투자 도메인 | `https://mockapi.kiwoom.com` |
| URL | `/api/dostk/stkinfo` |
| Format | JSON |
| Content-Type | application/json;charset=UTF-8 |

## 인증/헤더

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 샘플 | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 헤더 | `authorization` | 접근토큰 | String | Y | 1000 | - | 토큰 지정시 토큰타입("Bearer") 붙혀서 호출 <br> 예) Bearer Egicyx... |
| 헤더 | `cont-yn` | 연속조회여부 | String | N | 1 | - | 응답 Header의 연속조회여부값이 Y일 경우 다음데이터 요청시 응답 Header의 cont-yn값 세팅 |
| 헤더 | `next-key` | 연속조회키 | String | N | 50 | - | 응답 Header의 연속조회여부값이 Y일 경우 다음데이터 요청시 응답 Header의 next-key값 세팅 |
| 헤더 | `api-id` | TR명 | String | Y | 10 | - | 7자리 TR코드, ex) ka00001 |

## 요청

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 샘플 | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Body | `stk_cd` | 종목코드 | String | Y | 20 | 005930 | 거래소별 종목코드<br>(KRX:039490,NXT:039490_NX,SOR:039490_AL) |

## 응답

### 헤더

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 샘플 | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 헤더 | `cont-yn` | 연속조회여부 | String | N | 1 | - | 다음 데이터가 있을시 Y값 전달 |
| 헤더 | `next-key` | 연속조회키 | String | N | 50 | - | 다음 데이터가 있을시 다음 키값 전달 |
| 헤더 | `api-id` | TR명 | String | Y | 10 | - | 7자리 TR코드, ex) ka00001 |

### Body

| 위치 | 필드 | 이름 | 타입 | 필수 | 길이 | 샘플 | 설명 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Body | `base_pric` | 기준가 | String | N | 20 | - | 단위: 원 |
| Body | `buy_trde_ori_1` | 매수거래원1 | String | N | 20 | - | - |
| Body | `buy_trde_ori_2` | 매수거래원2 | String | N | 20 | - | - |
| Body | `buy_trde_ori_3` | 매수거래원3 | String | N | 20 | - | - |
| Body | `buy_trde_ori_4` | 매수거래원4 | String | N | 20 | - | - |
| Body | `buy_trde_ori_5` | 매수거래원5 | String | N | 20 | - | - |
| Body | `buy_trde_ori_nm_1` | 매수거래원명1 | String | N | 20 | - | - |
| Body | `buy_trde_ori_nm_2` | 매수거래원명2 | String | N | 20 | - | - |
| Body | `buy_trde_ori_nm_3` | 매수거래원명3 | String | N | 20 | - | - |
| Body | `buy_trde_ori_nm_4` | 매수거래원명4 | String | N | 20 | - | - |
| Body | `buy_trde_ori_nm_5` | 매수거래원명5 | String | N | 20 | - | - |
| Body | `buy_trde_qty_1` | 매수거래량1 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `buy_trde_qty_2` | 매수거래량2 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `buy_trde_qty_3` | 매수거래량3 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `buy_trde_qty_4` | 매수거래량4 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `buy_trde_qty_5` | 매수거래량5 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `cur_prc` | 현재가 | String | N | 20 | - | 단위: 원, 부호가 포함된 숫자 |
| Body | `flu_rt` | 등락율 | String | N | 20 | - | 단위: %, 부호 포함 소수점 둘째 자리까지 포맷된 백분율 |
| Body | `flu_smbol` | 등락부호 | String | N | 20 | - | - |
| Body | `pred_pre` | 전일대비 | String | N | 20 | - | 단위: 원, 부호가 포함된 숫자 |
| Body | `sel_trde_ori_1` | 매도거래원1 | String | N | 20 | - | - |
| Body | `sel_trde_ori_2` | 매도거래원2 | String | N | 20 | - | - |
| Body | `sel_trde_ori_3` | 매도거래원3 | String | N | 20 | - | - |
| Body | `sel_trde_ori_4` | 매도거래원4 | String | N | 20 | - | - |
| Body | `sel_trde_ori_5` | 매도거래원5 | String | N | 20 | - | - |
| Body | `sel_trde_ori_nm_1` | 매도거래원명1 | String | N | 20 | - | - |
| Body | `sel_trde_ori_nm_2` | 매도거래원명2 | String | N | 20 | - | - |
| Body | `sel_trde_ori_nm_3` | 매도거래원명3 | String | N | 20 | - | - |
| Body | `sel_trde_ori_nm_4` | 매도거래원명4 | String | N | 20 | - | - |
| Body | `sel_trde_ori_nm_5` | 매도거래원명5 | String | N | 20 | - | - |
| Body | `sel_trde_qty_1` | 매도거래량1 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `sel_trde_qty_2` | 매도거래량2 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `sel_trde_qty_3` | 매도거래량3 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `sel_trde_qty_4` | 매도거래량4 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `sel_trde_qty_5` | 매도거래량5 | String | N | 20 | - | 단위: 1주, 부호가 포함된 숫자 |
| Body | `stk_cd` | 종목코드 | String | N | 20 | - | - |
| Body | `stk_nm` | 종목명 | String | N | 40 | - | - |

## 예제

### Request

```json
{
  "stk_cd": "005930"
}
```

### Response

```json
{
  "stk_cd": "005930",
  "stk_nm": "삼성전자",
  "cur_prc": "95400",
  "flu_smbol": "3",
  "base_pric": "95400",
  "pred_pre": "0",
  "flu_rt": "0.00",
  "sel_trde_ori_nm_1": "",
  "sel_trde_ori_1": "000",
  "sel_trde_qty_1": "0",
  "buy_trde_ori_nm_1": "",
  "buy_trde_ori_1": "000",
  "buy_trde_qty_1": "0",
  "sel_trde_ori_nm_2": "",
  "sel_trde_ori_2": "000",
  "sel_trde_qty_2": "0",
  "buy_trde_ori_nm_2": "",
  "buy_trde_ori_2": "000",
  "buy_trde_qty_2": "0",
  "sel_trde_ori_nm_3": "",
  "sel_trde_ori_3": "000",
  "sel_trde_qty_3": "0",
  "buy_trde_ori_nm_3": "",
  "buy_trde_ori_3": "000",
  "buy_trde_qty_3": "0",
  "sel_trde_ori_nm_4": "",
  "sel_trde_ori_4": "000",
  "sel_trde_qty_4": "0",
  "buy_trde_ori_nm_4": "",
  "buy_trde_ori_4": "000",
  "buy_trde_qty_4": "0",
  "sel_trde_ori_nm_5": "",
  "sel_trde_ori_5": "000",
  "sel_trde_qty_5": "0",
  "buy_trde_ori_nm_5": "",
  "buy_trde_ori_5": "000",
  "buy_trde_qty_5": "0",
  "return_code": 0,
  "return_msg": "정상적으로 처리되었습니다"
}
```

## 연속조회/실시간/주의사항

- 연속조회는 응답 헤더 `cont-yn`, `next-key`가 문서에 있는 경우 해당 값을 다음 요청 헤더에 반영합니다.
- 실시간 API는 공식 문서의 API 설명과 요청/응답 필드 구조를 우선합니다.
- 빈 값은 공식 문서에 값이 없음을 의미합니다.

## 관련 오류

- [키움증권 오류코드](../errors.md)
